...
class NoiseGuidedDiffusion:
    ...
    def detect_details(self, image, sensitivity, smoothing):
        ...
        if isinstance(image, torch.Tensor):
            ...
            # Keep batch dimension but get first image
            image = image[0]
        ...
        # Ensure output shape matches input dimensions
        assert detail_mask.shape == (height, width), f"Shape mismatch: {detail_mask.shape} vs expected {(height, width)}"
    
        return detail_mask

    def apply(self, model, image, noise_scale, detail_sensitivity, smoothing, seed):
        # Get image dimensions from the input image tensor (BCHW format)
        ...
        assert detail_mask.shape == (height, width), "Detail mask shape mismatch"
        assert noise.shape == (height, width), "Noise shape mismatch"
        ...
        # Convert masks to torch tensors with correct dimensions (BCHW)
        ...
        assert detail_mask_tensor.shape == (1, 1, height, width), f"Detail mask tensor wrong shape: {detail_mask_tensor.shape}"
        assert noise_map_tensor.shape == (1, 1, height, width), f"Noise map tensor wrong shape: {noise_map_tensor.shape}"
